import {
  Map, CanvasLayer, MultiPolygon,
  Util, Coordinates, renderer,
  Extent,
  // @ts-ignore
} from 'maptalks';
// @ts-ignore
...
import { updateExtent } from '../../utils/geom';
...
export default class GlLayer extends CanvasLayer {
...
export class GlRenderer extends renderer.CanvasLayerRenderer {
  addGeometry(geometries: any) { // GeoJSON 即可，不需要构造要素对象
      ...
      if (geometry && geometry.coordinates) {
        switch (geometry.type) {
          case 'LineString':
            ...
            updateExtent(geometry.coordinates, min, max);
            break;
          case 'MultiLineString':
            for (let k = 0; k < geometry.coordinates.length; k++) {
              polylines.push(geometry.coordinates[k]);
              polylineFeatureIndices.push(i);
              updateExtent(geometry.coordinates[k], min, max);
            }
            break;
          case 'Polygon':
            polygons.push(geometry.coordinates);
            polygonFeatureIndices.push(i);
            updateExtent(geometry.coordinates[0], min, max);
            break;
          case 'MultiPolygon':
            for (let k = 0; k < geometry.coordinates.length; k++) {
              polygons.push(geometry.coordinates[k]);
              polygonFeatureIndices.push(i);
              updateExtent(geometry.coordinates[k][0], min, max);
            }
            break;
        }
      }
    ...
    const extent = new Extent(...min.concat(...max));
    console.log(extent);
  }
...
// @ts-ignore
ThreeLayer.registerRenderer('gl', GlRenderer);
